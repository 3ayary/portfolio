...
export default function Card({ title, description, image, tags, actions }) {
  return (
    ...
    <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md w-[400px] h-full transition-shadow">
      <div className="relative h-48">
        <Image src={image || "/placeholder.svg"} alt={title} fill className="object-cover w-[300px] h-[300px]" />
      </div>
      <div className="p-6 h-fit">
        <h3 className="text-xl font-semibold mb-2">{title}</h3>
        <p className="text-gray-600 mb-4 h-[70px]">{description}</p>
